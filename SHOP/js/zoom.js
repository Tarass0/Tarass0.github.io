...
function viewBigImg() {
    $(".goods__img img").click(function () { // Событие клика на маленькое изображение
        var img = $(this); // Получаем изображение, на которое кликнули
        var src = img.attr('src'); // Достаем из этого изображения путь до картинки
        $("body").append("<div class='wrap__view'>" + //Добавляем в тело документа разметку всплывающего окна
            "<div class='view__bg close'></div>" + // Блок, который будет служить фоном затемненным
            "<div class='wrap__img'>" + // Блок обгортка для збільшеного зображення та хрестика
            "<div class='view__dagger close'>" + // Блок для хрестика
            "<img src='../foto/dagger.svg'>"+ //картинка хрестика
            "</div>" + 
            "<img src=" + src + " class='view__img' />" + // Само увеличенное фото
            "</div>" + 
            "</div>");
        blockScroll('hidden'); // блокує скрол
        $(".wrap__view").fadeIn(500); // Медленно выводим изображение
        $(".close").click(function () { // Событие клика на затемненный фон	, по хрестику та дого обгортці  
            blockScroll('visible'); // розблоковує скрол 
            $(".wrap__view").fadeOut(500); // Медленно убираем всплывающее окно
            setTimeout(function () { // Выставляем таймер
                $(".wrap__view").remove(); // Удаляем разметку высплывающего окна
            }, 500);
        });
    });
}
//перевіряє ширину екрану і в залежності від ширини викликається певна функція
function checkWidth(width){
    switch (true) {
        case (width > 1025):
            //console.log("Викликається функція zoomImg();");
            //console.log("ширина більше 1025px");
            $('.goods__img img').off(); //видаляє попередній обробник 
            zoomImg();
            break;
        case (width < 1025):
            //console.log("Викликається функція  viewBigImg()");
            //console.log("ширина менше 1025px");
            $('.goods__img img').off(); //видаляє попередній обробник 
            viewBigImg();
            break;
        default:
...
$(window).bind("load resize", function () {
    ...
    resizeTimer = setTimeout(function () {
        ...
        checkWidth(width);
        //console.log("ресайз зупинився");
        //console.log("Ширина екрану на данний момент  = " + width);
    }, 250);
});
